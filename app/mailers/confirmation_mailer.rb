class ConfirmationMailer < MandrillMailer::TemplateMailer
  ...
  def send_confirmation(locations)
    ...
    mandrill_mail(
      ...
      recipient_vars: locations.map do |location|
        dt = DonationToken.create(user_id: location.user_id, location_id: location.id)
        ...
        { location.user.email =>
          {
            "USER_YES_DONATION_URL" => confirm_yes_url(dt.token, location.id),
            "USER_NO_DONATION_URL" => confirm_no_url(dt.token, location.id)
          }
        }
      end
     )
  ...
  def confirm_yes_url(token, location_id)
    default_host + "/confirm-donation?location_id=#{location_id}&token=#{token}"
  end

  def confirm_no_url(token, location_id)
    default_host + "/sorrynotthistime?location_id=#{location_id}&token=#{token}"
  end
